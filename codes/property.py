...
class SwitchBlockInfo(bpy.types.PropertyGroup):
    ...
    target_id: bpy.props.IntProperty(
        name=property_name("TargetID"), update=switch_block_update
    )  # type: ignore
...
class Property(bpy.types.PropertyGroup):
    color_file_path: bpy.props.StringProperty(
        name=property_name("Color File Path"), default=""
    )  # type: ignore
    ...
    brush_radius: bpy.props.IntProperty(
        name=property_name("笔刷半径"),
        description=property_name("方块笔刷半径（0 为单点）"),
        default=0,
        min=0,
        max=16,
    )  # type: ignore
    ...
    bpy.types.Scene.mods_dir = bpy.props.StringProperty(
        ...
        default=os.path.join(
            os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "temp"
        ),
    )
    bpy.types.Scene.jars_dir = bpy.props.StringProperty(
        name=property_name("jar文件路径"), default=os.path.join("mods")
    )
    ...
    bpy.types.Scene.saves_dir = bpy.props.StringProperty(
        ...
        default=os.path.join(
            os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "saves"
        ),
    )
    ...
    bpy.types.Scene.is_weld = bpy.props.BoolProperty(
    ...
    bpy.types.Scene.min_coordinates = bpy.props.IntVectorProperty(
        name=property_name("最小坐标"), size=3
    )
    bpy.types.Scene.max_coordinates = bpy.props.IntVectorProperty(
        name=property_name("最大坐标"), size=3
    )

    bpy.types.Scene.schem_size = bpy.props.IntVectorProperty(
        name=property_name("结构大小"), size=3
    )
    bpy.types.Scene.schem_location = bpy.props.IntVectorProperty(
        name=property_name("结构位置"), size=3
    )
    ...
    bpy.types.Scene.separate_vertices_by_blockid = bpy.props.BoolProperty(
        ...
        description=property_name(
            "将导入的结构按照方块类型分离，每种方块类型生成一个独立的 Blender 对象"
        ),
        default=False,
    )
    bpy.types.Scene.separate_vertices_by_chunk = bpy.props.BoolProperty(
        ...
        description=property_name(
            "将导入的结构按照 Minecraft 区块（16x16x16）分离，每个区块生成一个独立的 Blender 对象"
        ),
        default=False,
    )
    bpy.types.Scene.schem_filename = bpy.props.StringProperty(
        name=property_name(".schem文件名"), default="file"
    )
    ...
    bpy.types.Scene.mc_platform = bpy.props.EnumProperty(
        ...
        items=[
            enum_item(
                "java", property_name("Java Edition"), property_name("Java 版本")
            ),
            enum_item(
                "bedrock",
                property_name("Bedrock Edition"),
                property_name("基岩版（主机/手机）"),
            ),
        ],
        default="java",
    )
    ...
    bpy.types.Scene.world_name = bpy.props.StringProperty(
        name=property_name("World Name"), default="World1"
    )
    bpy.types.Scene.spawn_x = bpy.props.IntProperty(
        name=property_name("Spawn X"), default=0
    )
    bpy.types.Scene.spawn_y = bpy.props.IntProperty(
        name=property_name("Spawn Y"), default=64
    )
    bpy.types.Scene.spawn_z = bpy.props.IntProperty(
        name=property_name("Spawn Z"), default=0
    )
    ...
    def _true_false_enum_items():
        yes = property_name("是")
        no = property_name("否")
        return [enum_item("True", yes, yes), enum_item("False", no, no)]

    def _false_true_enum_items():
        yes = property_name("是")
        no = property_name("否")
        return [enum_item("False", no, no), enum_item("True", yes, yes)]
    ...
    # 整数
    bpy.types.Scene.max_entity_cramming = bpy.props.IntProperty(
        name=property_name("Max Entity Cramming"), default=12
    )
    bpy.types.Scene.snow_accumulation_height = bpy.props.IntProperty(
        name=property_name("Snow Accumulation Height"), default=1
    )
    bpy.types.Scene.spawn_radius = bpy.props.IntProperty(
        name=property_name("Spawn Radius"), default=10
    )
    bpy.types.Scene.players_sleeping_percentage = bpy.props.IntProperty(
        name=property_name("Players Sleeping Percentage"), default=0
    )
    bpy.types.Scene.random_tick_speed = bpy.props.IntProperty(
        name=property_name("Random Tick Speed"), default=0
    )
    bpy.types.Scene.command_modification_block_limit = bpy.props.IntProperty(
        name=property_name("Command Modification Block Limit"), default=32768
    )
    bpy.types.Scene.max_command_chain_length = bpy.props.IntProperty(
        name=property_name("Max Command Chain Length"), default=65536
    )
    bpy.types.Scene.day_time = bpy.props.IntProperty(
        name=property_name("Day Time"), default=16000
    )
    bpy.types.Scene.seed = bpy.props.IntProperty(
        name=property_name("Seed"), default=random.randint(0, 10000)
    )

    bpy.types.Scene.flySpeed = bpy.props.FloatProperty(
        name=property_name("FlySpeed"), default=0.05
    )
    bpy.types.Scene.flying = bpy.props.BoolProperty(
        name=property_name("Flying"), default=False
    )
    bpy.types.Scene.instabuild = bpy.props.BoolProperty(
        name=property_name("instabuild"), default=True
    )
    bpy.types.Scene.invulnerable = bpy.props.BoolProperty(
        name=property_name("invulnerable"), default=True
    )
    bpy.types.Scene.mayBuild = bpy.props.BoolProperty(
        name=property_name("mayBuild"), default=True
    )
    bpy.types.Scene.mayfly = bpy.props.BoolProperty(
        name=property_name("mayfly"), default=True
    )

    bpy.types.Scene.walkSpeed = bpy.props.FloatProperty(
        name=property_name("walkSpeed"), default=0.1
    )

    bpy.types.Scene.luck = bpy.props.FloatProperty(
        name=property_name("幸运值"), default=0, min=-1024, max=1024
    )
    bpy.types.Scene.max_health = bpy.props.FloatProperty(
        name=property_name("最大生命值"), default=20, min=1, max=1024
    )
    bpy.types.Scene.knockback_resistance = bpy.props.FloatProperty(
        name=property_name("击退抗性"), default=0, min=0, max=1
    )
    bpy.types.Scene.movement_speed = bpy.props.FloatProperty(
        name=property_name("移动加速度"), default=0, min=0, max=1024
    )
    bpy.types.Scene.armor = bpy.props.FloatProperty(
        name=property_name("盔甲值"), default=0, min=0, max=30
    )
    bpy.types.Scene.armor_toughness = bpy.props.FloatProperty(
        name=property_name("盔甲韧性"), default=0, min=0, max=20
    )
    bpy.types.Scene.attack_damage = bpy.props.FloatProperty(
        name=property_name("攻击伤害"), default=0, min=0, max=2048
    )
    bpy.types.Scene.attack_speed = bpy.props.FloatProperty(
        name=property_name("攻击速度"), default=0, min=0, max=1024
    )
...
def unzip_mods_files():
    ...
    folder_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "mods"
    )
                        ...
                        with zip_ref.open("META-INF/mods.toml") as mods_toml_file:
                            ...
                            if "mods" in mods_toml_data:
                                for mod_entry in mods_toml_data["mods"]:
                                    ...
                                    icon = mod_entry.get("logoFile", "").replace(
                                        "/", "\\"
                                    )  # 添加默认值，防止没有 "logoFile" 字段时报错
                                    name = mod_entry.get(
                                        "displayName", ""
                                    )  # 添加默认值，防止没有 "displayName" 字段时报错
                                    description = mod_entry.get(
                                        "description", ""
                                    )  # 添加默认值，防止没有 "description" 字段时报错
                            else:
                ...
                if mod_id:
                    ...
                    for member in zip_ref.namelist():
                        try:
                            ...
                            if member.startswith("assets/") or member.startswith(
                                "data/"
                            ):
                                # 构造解压路径
...
def unzip_resourcepacks_files():
    # 指定的文件夹路径
    folder_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "resourcepacks"
    )
    ...
    # 临时文件夹路径
    temp_dir = os.path.join(
        os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "temp", "资源包"
    )
...
class UnzipModOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        bpy.app.timers.register(
            functools.partial(self.check_thread, thread), first_interval=1.0
        )
