...
### .schem 文件
- **导入 .schem 文件 (主按钮)**: 打开文件选择器导入 `.schem` 文件。当方块数量超过阈值时自动启用多进程并行导入。
- **导入 Schem 选项**:
...
## 2. 导出 (Export)
将 Blender 场景中的方块数据导出为文件或写回 Minecraft 存档。

- **文件名**: 设置导出的 `.schem` 文件名（默认 "file"）。
- **导出结构**: 将选中的对象导出为 `.schem` 文件。
- **选择世界**: 下拉列表，选择目标 Minecraft 存档（需在 Config 设置存档路径）。
- **结构大小**: 显示当前选中对象的长宽高（Blender 坐标系）。
- **位置**: 显示导出原点坐标。
- **计算结构大小**: 刷新上方的尺寸和位置数据。
- **导出结构到存档**: 将选中对象的方块数据直接写入选定存档（按当前版本配置转换并保存世界）。
...
- **世界类型**: 默认、超平坦等。
- **极限模式**: 是否启用极限模式（仅一个生命）。
- **允许指令**: 是否允许玩家使用命令方块和斜杠命令。
- **突破限高**: 是否突破 320 方块的高度限制至 2032。
- **时间**: 世界初始时间。
- **种子**: 世界生成种子。
- **创建存档**: 生成存档文件。

创建后文件位置：`MBM_workflow/saves/<存档名称>/level.dat`。

### 更多设置 (More Level Settings)
- **命令限制**: 修改最大连锁长度等高级参数。
- **环境参数**: 随机刻速度 (Random Tick Speed)、雪堆积高度等。
- **最大实体挤压数**: 一方块空间内挤压到一起的实体的最大数量。
...
- **合并+规范化点云**: 合并多个点云对象并规整到整数网格，去重重叠点。
- **可视化编辑**:
...
## 6. 插件偏好设置 (Addon Preferences)

**位置**: Edit > Preferences > Add-ons > MBM_Workflow

### 多进程设置
- **ProcessNumber**: 最大进程数（默认 6），同时处理的区块数量。推荐值为 CPU 核心数的一半。
- **MinSize**: 多进程阈值（默认 1,000,000），导入的方块数超过此值时自动启用多进程。
- **刷新**: 自动根据 CPU 核心数重置上述参数。
