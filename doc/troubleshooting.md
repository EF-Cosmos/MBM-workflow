...
**解决方案**：
- 插件会自动检测方块数量，超过阈值（默认 100 万）时自动启用多进程并行导入。
- 在插件偏好设置中调整 `MinSize` 阈值和 `ProcessNumber` 进程数。
- 在导入面板勾选 `按区块分离`，减少单个对象的顶点数量。
- 大型场景建议分区域导入并分批处理。

### 多进程导入子进程异常退出
**现象**：控制台显示 `[MBM] 子进程 X 异常退出，返回码: 1`。
...
**解决方案**：
1. 检查 Blender 控制台的子进程输出日志。
2. 确认 `schemcache/` 目录存在且可写。
3. 减小 `ProcessNumber` 以降低内存占用。
4. 清理 `schemcache/` 中的旧 `var.json` 和 `chunk*.pkl` 文件后重试。
